...
def draw(results: List[List[ItemModel]], color_index):
    mesh = []
    clr = color_index[1]
    ...
    for pieces in results:
        ...
        colors = []
        for each in pieces:
            positions.append([each.x0, each.y0, each.z0])
            sizes.append([each.length, each.width, each.height])
            for i in range(len(sorted_pieces)):
                if set(sizes[-1]) == sorted_pieces[i]:
                    colors.append(clr[i])
