...
def evaluate(population, truck_dimension, boxes, total_value):
    ...
    for key, individual in population.items():
        ...
        for box_number, r in zip(individual['order'], individual['rotate']):
            dblf = sorted(dblf, key=operator.itemgetter(3))
            dblf = sorted(dblf, key=operator.itemgetter(5))
            dblf = sorted(dblf, key=operator.itemgetter(4))
            for pos in dblf:
        ...
        ft[key] = fitness
        population[key]['fitness'] = copy.deepcopy(fitness)
