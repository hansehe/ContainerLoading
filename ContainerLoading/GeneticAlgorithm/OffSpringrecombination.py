...
def recombine(parents):
    ...
    for x in range(0, len(parents), 2):
        ...
        o1 = copy.deepcopy(parents[k1]['order'])
        r1 = copy.deepcopy(parents[k1]['rotate'])
        keys.remove(k1)
        k2 = random.choice(keys)
        o2 = copy.deepcopy(parents[k2]['order'])
        r2 = copy.deepcopy(parents[k2]['rotate'])
        keys.remove(k2)
        ...
        for k in range(len(r1)):
            if cr2[pos] == -1:
                cr2[pos] = r1[k]
                pos = (pos + 1) % len(r1)
        offsprings[x] = {'order': copy.deepcopy(co1), 'rotate': copy.deepcopy(cr1)}
        offsprings[x + 1] = {'order': copy.deepcopy(co2), 'rotate': copy.deepcopy(cr2)}
    return offsprings
